...
redis_client = redis.StrictRedis(host=REDIS_HOST, db=REDIS_DATABASE, decode_responses=True)

parser = reqparse.RequestParser()
parser.add_argument('name', required=True, help="name of the food cannot be blank")
parser.add_argument('restaurant', required=True, help="restaurant of the food cannot be blank")
parser.add_argument('price', required=True, help="calories must be a number")
parser.add_argument('calories', type=int, required=True, help="calories must be a number")
parser.add_argument('type', required=True, help="type of the food cannot be blank")

class FoodAPI(Resource):
    def get(self, name):
        fooc = Food.get_food(name)
        if food:
